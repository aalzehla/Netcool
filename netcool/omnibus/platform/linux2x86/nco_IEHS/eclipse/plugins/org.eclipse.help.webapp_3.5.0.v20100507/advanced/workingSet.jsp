...
<% 
	...
	List<String> wsNameList = data.getWorkingSetNameList();
%>
...
// Preload images
var imagesDirectory = "../<%=pluginVersion%>/advanced/<%=prefs.getImagesDirectory()%>";
var minus = new Image();
...
var wsNameList = [<%for(String name: wsNameList){%>"<%=UrlUtil.JavaScriptEncode(name)%>",<%}%>];
var nameAlreadyExist = "<%=UrlUtil.JavaScriptEncode(ServletResources.getString("nameAlreadyExist", request))%>";
...
function doSubmit()
{
	try
	{
        ...
        for(var i in wsNameList){
            if(workingSet == wsNameList[i]){
            	if(<%=data.isEditMode()%> && workingSet == "<%=UrlUtil.JavaScriptEncode(data.getWorkingSetName())%>"){
                    break;
                }
                alert(nameAlreadyExist);
                return false;
            }
        }
...
function enableOK() {
<%
	if(data.isEditMode() && data.getWorkingSet() != null){
		if(!data.getWorkingSet().isEditable()){
%>
...
<body role="main" dir="<%=direction%>" onload="onloadHandler()"  onresize = "sizeContainer()" <%if(data.isMozilla()){%>onkeydown="escKeydownHandler(event);"<%}%>>
<h1 id="workingSetTitle" style="display:none;"><%=ServletResources.getString(data.isEditMode()?"EditWorkingSet":"NewWorkingSet", request)%></h1>
<form onsubmit="doSubmit();return false;" role="form" aria-labelledby="workingSetTitle">
	<table role="presentation" id="wsTable" width="100%" cellspacing=0 cellpading=0 border=0 align=center >
    ...
    <table role="presentation" width="100%" cellspacing=0 cellpading=0 border=0 align=center style="table-layout:fixed;">
        <tr>
            <td>
		    ...
		    <td>
               ...
               %>
				    <div class="book" id='<%="id"+i%>' >
					   <img role="button" id='<%="img"+i%>' alt="<%=ServletResources.getString("bookClosed", request)%>" title="<%=ServletResources.getString("bookClosed", request)%>" src="../<%=pluginVersion%>/advanced/<%=prefs.getImagesDirectory()%>/plus.gif" onclick="collapseOrExpand('<%=i%>')">
               ...
               <div id="criteriaContainer">
                   ...
                   %>
				    <div class="criterion" id='<%="id_criterion"+i%>' >
					   <img role="button" id='<%="img_criterion"+i%>' alt="<%=ServletResources.getString("criterionClosed", request)%>" title="<%=ServletResources.getString("criterionClosed", request)%>" src="../<%=pluginVersion%>/advanced/<%=prefs.getImagesDirectory()%>/plus.gif" onclick="collapseOrExpand('_criterion'+'<%=i%>')">
...
<div id="buttonBar" >
	<table valign="bottom" align="<%=isRTL?"left":"right"%>">
		<tr id="buttonsTable" valign="bottom"><td valign="bottom" align="<%=isRTL?"left":"right"%>">
  			<table role="presentation" cellspacing=10 cellpading=0 border=0 style="background:transparent;">
				<tr>
					<td>
