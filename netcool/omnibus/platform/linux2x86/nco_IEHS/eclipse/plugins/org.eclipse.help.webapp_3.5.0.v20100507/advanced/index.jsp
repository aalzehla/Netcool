...
function IObserver(notifyFun) {
	this.notify = notifyFun;
}

function VCCSubjectClass() {
	this.logonObserver = new Array()/*<Observer>*/;
}

VCCSubjectClass.prototype.registerLogonObserver = function (/*Observer*/ obj) {
	this.logonObserver[this.logonObserver.length] = obj;
};

VCCSubjectClass.prototype.notifyLogonObserver = function () {
	var obAmount = this.logonObserver.length;
	for(var i = 0; i < obAmount; i++) {
		this.logonObserver[i].notify();
	}
};

var VCCSubject = new VCCSubjectClass();
</script>

</head>

<frameset role="main" id="indexFrameset" onload="onloadHandler()" rows="<%="0".equals(data.getBannerHeight())?"":data.getBannerHeight()+","%>24,*<%=data.getFooterRowText()%>"  frameborder="0" framespacing="0" border=0 spacing=0>
<%
...
%>
	<frame name="HelpToolbarFrame" title="<%=ServletResources.getString("ignore", ServletResources.getString("HelpToolbarFrame",request), request)%>" src='<%="advanced/helpToolbar.jsp"+UrlUtil.htmlEncode(data.getQuery())%>' marginwidth="0" marginheight="0" scrolling="no" frameborder="0" noresize=0>
	<frame name="HelpFrame" title="<%=ServletResources.getString("ignore", ServletResources.getString("HelpFrame",request), request)%>" src='<%="advanced/help.jsp"+UrlUtil.htmlEncode(data.getQuery())%>' marginwidth="0" marginheight="0" scrolling="no" frameborder="0" >
<%
