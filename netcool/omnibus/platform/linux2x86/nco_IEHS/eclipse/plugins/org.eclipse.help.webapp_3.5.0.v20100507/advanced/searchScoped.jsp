...
<% 
	...
	String scopeName = UrlUtil.htmlEncode(data.getScope());
%>
...
<title><%=ServletResources.getString("SearchFrame", request)%></title>
...
function rescope() {
    if (parent.parent.HelpFrame && 
		...
		if (viewsFrame.vcc && viewsFrame.vcc.vccViewFrame) {
			var vccView = viewsFrame.vcc.vccViewFrame;
			vccView.repaint();
	    }
		doSearch(null, true);
	}
...
<body dir="<%=direction%>" onload="onloadHandler()"  onunload="closeAdvanced()" role="region" aria-label="<%=ServletResources.getString("SearchFrame", request)%>">
	<h1 id="searchTitle" style="display:none;"><%=ServletResources.getString("SearchFrame", request)%></h1>
	<form  name="searchForm"   onsubmit="doSearch()" role="search" aria-labelledby="searchTitle">
		<table role="presentation" id="searchTable" align="<%=isRTL?"right":"left"%>" valign="middle" cellspacing="0" cellpadding="0" border="0">
			<tr nowrap  valign="middle">
				...
				<td >
					<input type="submit" onclick="this.blur();doSearch()" value='<%=ServletResources.getString("GO", request)%>' id="go" alt='<%=ServletResources.getString("GO", request)%>' title='<%=ServletResources.getString("GO", request)%>'>
				...
				<td nowrap>
					<input type="hidden" name="workingSet" value='<%=scopeName%>'>
					<div id="scope" ><%=scopeName%></div>
				</td>
			</tr>
