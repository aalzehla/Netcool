...
<frameset role="search" aria-label="<%=ServletResources.getString("helpToolbarFrame", request)%>" id="helpToolbarFrameset" cols="<%=frameData.getHelpToolbarFrameSizes()%>" frameborder=0" framespacing="0" border="0" spacing="0">
<% if (isRTL) {
if(null != frame){%>
    <frame name="<%=frame.getName()%>" id="<%=frame.getID()%>" title="<%=frame.getTitle(UrlUtil.getLocaleObj(request, null))%>" src="<%= frameData.getUrl(frame) %>" <%=frame.getFrameAttributes()%> >
<%}%>   	
