...
{
	//oldActive in IE is like '../normal html file.html'. But its topic is like '../normal%20html%20file.html'
	//so need to change the oldActive format to keep consistent with its topic so that it can execute method of 'focusOnItem()'
	if(oldActive){
        oldActive.href = oldActive.href.replace(/ /g,"%20");
	}
...
function stripParams(href) {
    ...
    if (qMark < 0) {
    	...
    	var topicHref = href.substring(0,qMark);
    	var anchorIndex = href.indexOf('#');
    	if(anchorIndex < 0){
    		return topicHref;
    	}else{
    		var anchorHref = href.substring(anchorIndex);
        	return topicHref + anchorHref;
    	}
