...
var linksArray = new Array ("linktoc", "linkindex", "linksearch", "linkbookmarks", "linkvcc");
...
%>
	...
	<%=views[i].getName()%>.src = "<%=image%>";
	<%}else{%>
	<%=views[i].getName()%>.src = "../<%=pluginVersion%>/advanced/<%=image%>";
<%
	}}}
%>
...
<body dir="<%=direction%>" onload="showTab('<%=data.getVisibleView()%>')" role="region" aria-label="<%=ServletResources.getString("Tabs", request)%>">
  <h1 id="tabsTitle" style="display:none;"><%=ServletResources.getString("Tabs", request)%></h1>
  <table role="navigation" aria-labelledby="tabsTitle" cellspacing="0" cellpadding="0" border="0" width="100%" height="100%" valign="middle">
   <tr>
...
%>
	<td  role="button" title="<%=UrlUtil.htmlEncode(title)%>" 
	     align="center"  
	     valign="middle"
	     class="tab" 
	     id="<%=views[i].getName()%>" 
	     ...
	     <a  href='javascript:parent.showView("<%=views[i].getName()%>");'  
	         ...
	         <img alt="<%=UrlUtil.htmlEncode(title)%>" 
	              ...
	              src="<%=image%>"
	              <%}else{%>
	              src="../<%=pluginVersion%>/advanced/<%=data.getImageURL(views[i])%>"
	              <%}%>
