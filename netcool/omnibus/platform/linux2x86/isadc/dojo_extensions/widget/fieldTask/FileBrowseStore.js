...
//
dojo.provide("widgets.fieldTask.FileBrowseStore");

dojo.declare("widgets.fieldTask.FileBrowseStore", null, {
	constructor: function(/*Object*/args){
		...
		var applet=dijit.byId("loadingAppletID").getISAApplet();
		this.appletLoader=applet;
	},
	...
	_assertIsItem: function(/* item */ item){
		...
		if(!this.isItem(item)){ 
			throw new Error("widgets.fieldTask.FileBrowseStore: a function was passed an item argument that was not an item");
		}
	},
	...
	_assertIsAttribute: function(/* attribute-name-string */ attribute){
		...
		if(typeof attribute !== "string"){ 
			throw new Error("widgets.fieldTask.FileBrowseStore: a function was passed an attribute argument that was not an attribute name string");
		}
	},
	...
	loadItem: function(keywordArgs){
		...
		console.debug("using applet to load data from loadItems");
		...
		var data =this._appletMethod(xhrData);
    ...
    //This is the change that was made to call the applet function and replace the xhrget() call
	_appletMethod: function(data){
		//summary:
		//    this method makes a call to the applet to retrieve the tree information
		//
		//  if the path is null, we retrieve the drives (node)
		// if the path is non null, we retrieve the information related to the path
		console.debug("In the applet Method ...");		
		useApplet=true;		
		var input=data.content;		
		
		if(input.path==null){
			var dirsOnly=false;
			//console.debug=("Input options: "+input.options);
			if(input.options && input.options=='["dirsOnly"]')
				dirsOnly=true;

			var rootPathDescription=this.appletLoader.getRootsAsJson(dirsOnly);
			console.debug(rootPathDescription);
			return dojo.fromJson(rootPathDescription);
		} else{
			var dirsOnly=false;
			...
			var pathAsJSON = this.appletLoader.getPathAsJson(input.path, dirsOnly);
			console.debug(pathAsJSON);
			return dojo.fromJson(pathAsJSON);
		}
	},
	...
	fetch: function(request){
        ...
        //reqParams Object {query=root}
		console.debug("Running applet method from fetch()");
		var data=this._appletMethod(getArgs);
		self._processResult(data, request);
		
		//Add your javascript call here
		//var deferred = dojo.xhrGet(getArgs);

		//Process the result of the javascript by making call to self._processResult
		//deferred.addCallback(function(data){
		//	self._processResult(data, request);
		//	});
		
		//If something goes wrong, pass to this call here.
		//deferred.addErrback(function(error){
		//	if(request.onError){
	...
	fetchItemByIdentity: function(keywordArgs){
		...
		console.debug("using applet to load data from fetchItemByIdentity()");
		this._appletMethod(xhrData);
